...
\begin{lstlisting}[language=bash]
  ...
  6330
\end{lstlisting}%$
erfährt man, dass exakt 6330 Zeilen des Dokumentes das Wort ``road''
nicht enthält. Das flag \texttt{-i} sorgt dafür, dass \texttt{grep} die
Groß-/Kleinschreibung ignoriert, \texttt{-v} sorgt dafür, dass nur
Zeilen ohne Match ausgegeben werden, \texttt{-c} Z\"ahlt die von
\texttt{grep} ausgegebenen Zeilen.
\newsubsubproblem
Der \texttt{grep} Befehl wandelt zunächst die Datei in eine Liste
ihrer Wörter um. Danach werden die Wörter mit \texttt{sort} gruppiert
damit sie mit \texttt{uniq -c} zusammengefasst werden können und die
Anzahl ihrer Vorkommnisse bestimmt werden kann. Um die häufigsten 10
zu ermitteln wird dann mit \texttt{sort -n -r} nach den von
\texttt{uniq} hinzugefügten Anzahlen absteigend sotiert und
